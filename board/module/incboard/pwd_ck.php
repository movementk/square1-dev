...
$searchVal = "Category=".$Category."&order_type=RegDate&align=".$align."&sF=".$sF."&sT=".$sT."&date_idx=".$date_idx."&mode=".$mode."&workType=".$BoardName;
...
if($_REQUEST["BoardIdx"] != ""){	
	if($board_code=="board_delete"){
		...
		if($row[cnt] > 0){
			err_back("답변글이 있으므로 삭제하실 수 없습니다.");
		}
		...
		} else if (($user['ID'] && $user['ID']==$UserID) || $is_admin || $upw == $UserPw){
			$f_row = get_file($mode,$Idx);
			for($i=0;$i<$f_row[count];$i++){
				if(!empty($f_row[$i][file_source]) && file_exists($dir."/upload/".$BoardName."/".$f_row[$i]["file_source"])){
					@unlink($dir."/upload/".$BoardName."/".$f_row[$i]["file_source"]);
		...
		if ($UserPw==$upw || $is_admin){
			...
			if($BoardName == "customer" || $BoardName == "contact"){
				$sql_common = " sido = '$sido', gugun = '$gugun', store = '$store', phone = '$phone',";
			}

			if($BoardName == "counsel2"){
				$phone = $phone1."-".$phone2."-".$phone3;
				$program = $p1."|".$p2."|".$p3."|".$p4."|".$p5;
				$sql_common = " phone = '$phone', pcnt = '$pcnt', grade = '$grade', term = '$term', program = '$program', ";
			}
...
if($board_code=="board_delete"){
	if($Re_url){
		echo "<script>location.href='".$Re_url."?".$searchVal."';</script>";
	}
	exit;
} else {
	if($Re_url){
		if($BoardName == "customer" || $BoardName == "contact" || $BoardName == "materials"){
			err_move("접수되었습니다. 빠른시간내에 답변해드리겠습니다.",$Re_url);
		} else {
			echo "<script>location.href='".$Re_url."?board_code=board_view&board_idx=".$Idx."&workType=".$BoardName."';</script>";
		}
	}
	exit;
}
