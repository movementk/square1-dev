...
include $_SERVER["DOCUMENT_ROOT"]."/board/config/common_top.php";
...
//print_r2($_REQUEST);
$getParam = "?board_code=board_view&board_idx=".$BoardIdx."&page=".$page."&start_page=".$start_page."&Category=".$Category."&sT=".$sT."&sF=".$sF."&workType=".$workType."&mode=".$mode;

if($date_idx){
	$getParam = "?board_code=board_view&board_idx=".$BoardIdx."&page=".$page."&start_page=".$start_page."&Category=".$Category."&sT=".$sT."&sF=".$sF."&workType=".$workType."&mode=".$mode."&category=board&date_idx=".$date_idx;
}
...
if(!$counsel){
	$sql = " select * from ".$mode." where BoardIdx = '".$BoardIdx."' ";
	$row = sql_fetch($sql);
	if(!$row[BoardIdx]){
...
switch($cmode){
	case "d" :
		...
		if($is_admin){
			;
		} else if($comment_member[UserLevel] > $member[UserLevel]){
	...
	case "c" :
		if ($CommentIdx) {
			...
			$sql = "select max(comment_cnt) as max_comment from ".$comment_table." where BoardIdx = '".$BoardIdx."' ";
			$row = sql_fetch($sql);
			$row[max_comment] += 1;
...
switch($cmode){
	...
	case "c" : 
		if(!$works) $works = "입력";
		if($is_ajax){
			GetAlert("댓글이 {$works}되었습니다.","SCRIPTS","parent.comment_load('".$BoardIdx."','".$cpage."');");
		} else {
			GetAlert("댓글이 {$works}되었습니다.",$url.$getParam);
		}
		break;
}

include $_SERVER["DOCUMENT_ROOT"]."/board/common/common_bottom.php";
?>
