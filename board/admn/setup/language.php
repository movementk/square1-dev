...
$mode = $site_prefix."language_category";
...
<div id="container">
	<div class="content_view">
		<div class="con_title">봉사언어관리</div>
		...
		<table class="write_table">
			...
			<tbody>
			<tr>
				<th><label>언어명</label></th>
				<td><input type="text" class="input" name="language_name" exp title="등급명" value="<?=$arow["language_name"]?>" onchange="dupe_reset()">&nbsp;<button type="button" class="btn_a_n" onclick="language_dupe_ck();">중복확인</button><span id="span_id_ck" style="color:red;font-weight:bold;padding-left:10px;">※ 한글 2글자 또는 영문 4글자이상 입력하시기 바랍니다.</span></td>
				<th><label>설명</label></th>
				<td><input type="text" class="input wd600" name="language_text" exp title="설명" value="<?=$arow["language_text"]?>"></td>
			</tr>
			</tbody>
		</table>

		<div class="btn_group">
			<button type="button" class="btn_a_b" onclick="form_ck();"><?=$workType=="LI"?"등 록":"수 정"?></button>
		</div>
		</form>

		<span id="span_id_ck" style="color:red;font-weight:bold;padding-left:10px;">※ 언어명 삭제시 기존 회원정보는 업데이트되지 않습니다.</span>
		<table class="list_table mt15">
			...
			for ($i=0; $row=sql_fetch_array($result); $i++){
				$msql = " select count(*) as cnt from ".$memberdb." where m_flc_no = '".$row["idx"]."' ";
				$mrow = sql_fetch($msql);
			?>
			<tr>
				<td><?=$i+1;?></td>
				...
				<td><?=$row["language_text"]?></td>
				<td><?=date("Y-m-d",$row["wr_datetime"])?></td>
				<td>
					<div class="floatL pt5" style="width:100%;"><button type="button" class="mbtn_a_n" onclick="admin_modify('LM','<?=$row["idx"]?>');">수 정</button></div>
					<div class="floatL pt5 pb5" style="width:100%;"><button type="button" class="mbtn_a_b" onclick="admin_modify('LD','<?=$row["idx"]?>');">삭 제</button></div>
				</td>
			</tr>
			<?
				$num--;
...
function language_dupe_ck(){
	var id_dupe_ck = "N";
	var id_dupe_str = "";
	jQuery.ajax({
		url: "/board/admn/setup/ajax_dupe_check.php",
		type : "POST",
		data: "workType=LAN&str_limit=4&mode=<?=$mode?>&val="+$("input[name='language_name']").val()+"&old_name=<?=$arow['language_name']?>",
