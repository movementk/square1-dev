...
$mode = $site_prefix."member_category";
...
<div id="container">
	<div class="content_view">
		<div class="con_title">회원등급관리</div>
		...
		<table class="write_table">
			...
			<tbody>
			<tr>
				<th><label>등급명</label></th>
				<td><input type="text" class="input" name="grade_name" exp title="등급명" value="<?=$arow["grade_name"]?>" onchange="dupe_reset()">&nbsp;<button type="button" class="btn_a_n" onclick="grage_dupe_ck();">중복확인</button><span id="span_id_ck" style="color:red;font-weight:bold;padding-left:10px;">※ 한글 2글자 또는 영문 4글자이상 입력하시기 바랍니다.</span></td>
				<th><label>설명</label></th>
				<td><input type="text" class="input wd600" name="grade_text" exp title="설명" value="<?=$arow["grade_text"]?>"></td>
			</tr>
			</tbody>
		</table>

		<div class="btn_group">
			<button type="button" class="btn_a_b" onclick="form_ck();"><?=$workType=="MGI"?"등 록":"수 정"?></button>
		</div>
		</form>
		<span id="span_id_ck" style="color:red;font-weight:bold;padding-left:10px;">※ 등급명 삭제시 기존 회원정보는 업데이트되지 않습니다.</span>
		<table class="list_table mt15">
			...
			<tr>
				...
				<td>
					<div class="floatL pt5" style="width:100%;"><button type="button" class="mbtn_a_n" onclick="admin_modify('MGM','<?=$row["idx"]?>');">수 정</button></div>
					<div class="floatL pt5 pb5" style="width:100%;"><button type="button" class="mbtn_a_b" onclick="admin_modify('MGD','<?=$row["idx"]?>');">삭 제</button></div>
				</td>
			</tr>
			<?
				$num--;
...
function grage_dupe_ck(){
	var id_dupe_ck = "N";
	var id_dupe_str = "";
	jQuery.ajax({
		url: "/board/admn/setup/ajax_dupe_check.php",
		type : "POST",
		data: "workType=MG&str_limit=4&mode=<?=$mode?>&val="+$("input[name='grade_name']").val()+"&old_name=<?=$arow['grade_name']?>",
