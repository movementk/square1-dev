...
$mode = $site_prefix."case_category";
...
<div id="container">
	<div class="content_view">
		<div class="con_title">봉사자이야기 사례관리</div>
		...
		<table class="write_table">
			...
			<tbody>
			<tr>
				<th><label>사례명</label></th>
				<td><input type="text" class="input" name="case_name" exp title="등급명" value="<?=$arow["case_name"]?>" onchange="dupe_reset()">&nbsp;<button type="button" class="btn_a_n" onclick="job_dupe_ck();">중복확인</button><span id="span_id_ck" style="color:red;font-weight:bold;padding-left:10px;">※ 한글 2글자 또는 영문 4글자이상 입력하시기 바랍니다.</span></td>
				<th><label>설명</label></th>
				<td><input type="text" class="input wd600" name="case_text" exp title="설명" value="<?=$arow["case_text"]?>"></td>
			</tr>
			</tbody>
		</table>

		<div class="btn_group">
			<button type="button" class="btn_a_b" onclick="form_ck();"><?=$workType=="CI"?"등 록":"수 정"?></button>
		</div>
		</form>

		<span id="span_id_ck" style="color:red;font-weight:bold;padding-left:10px;">※ 사례명 삭제시 기존 봉사자이야기 게시글은 업데이트되지 않습니다.</span>
		<table class="list_table mt15">
			...
			<tr>
				...
				<td>
					<div class="floatL pt5" style="width:100%;"><button type="button" class="mbtn_a_n" onclick="admin_modify('CM','<?=$row["idx"]?>');">수 정</button></div>
					<div class="floatL pt5 pb5" style="width:100%;"><button type="button" class="mbtn_a_b" onclick="admin_modify('CD','<?=$row["idx"]?>');">삭 제</button></div>
				</td>
			</tr>
			<?
				$num--;
...
function job_dupe_ck(){
	...
	jQuery.ajax({
		...
		type : "POST",
		data: "workType=CASES&str_limit=4&mode=<?=$mode?>&val="+$("input[name='case_name']").val()+"&old_name=<?=$arow['case_name']?>",
