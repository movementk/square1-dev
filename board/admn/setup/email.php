...
<div id="container">
	<div class="content_view">
		<div class="con_title">이메일무단수집거부</div>
		<form name="agree_form" method="post" action="/board/admn/_proc/setup/_admin_proc.php">
		<input type="hidden" name="workType" value="EMAIL">
		<input type="hidden" name="URI" value="<?=$_SERVER['REQUEST_URI']?>">
		<input type="hidden" name="idx" value="<?=$row["idx"]?>">
		<textarea style="width:100%;height:450px;" name="agree3" id="agree3" ><?=$row["agree3"]?></textarea>
		<textarea style="display:none;" name="agree2" id="agree2" ><?=$row["agree2"]?></textarea>
...
function board_check(){
	...
	oEditors.getById["agree3"].exec("UPDATE_CONTENTS_FIELD", []);
	f.submit();
}
</script>
<?
