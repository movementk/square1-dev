...
if($brow["BoardName"] == "prizewinner"){
	$esql = " select * from ".$site_prefix."board_square1 where BoardIdx = '".$eidx."' ";
	$erow = sql_fetch($esql);
...
switch($workType){
	...
	case "M":
		...
		if($bd1 && $frow["bd10"] == "N"){
			...
			$from_name = "MTOS Admin";
			$from_mail = "admin@".$_SERVER['HTTP_HOST'];
			$to_mail = $UserEmail;
			$mail_subject = $UserName."님, MTOS Solutions입니다.";
			$mail_content = "MTOS Solutions에 문의하신 글에 답변이 등록되었습니다. <a href='http://".$_SERVER['HTTP_HOST']."/cscenter/qna.php?qidx=".$idx."'>[바로가기]</a>를 누르시면 확인하실 수 있습니다.";
			mailer($from_name,$from_mail,$to_mail,$mail_subject,$mail_content,1);
