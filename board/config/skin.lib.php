...
if (!function_exists("makeThumbs")) {
	...
	function makeThumbs($oriPath, $oriFileName, $thmWidth="", $thmHeight="", $thmFolder="", $thmAlt="", $errorFilePrt = "") {
		if(empty($errorFilePrt)) $errorFilePrt = "<img src='/noimg.jpg'>";
		$oriFile = $oriPath . "/" . $oriFileName;
		if (is_file($oriFile) == false) return $errorFilePrt."원본없음"; // 원본 부재

	//	$thmPath = $oriPath . "/".$thmFolder;
		$thmPath = $thmFolder;
		...
		if (file_exists($thmFile)) { // 썸네일 유무
			...
			if (is_dir($oriPath . "/".$thmFolder."/")) {
				$array1 = $array2 = array();
				if ($dh = opendir($oriPath . "/".$thmFolder."/")) {
				...
				foreach ($array_diff as $k => $v) {
					if (is_file($oriPath . "/".$thmFolder."/" . $v)) @unlink($oriPath . "/".$thmFolder."/" . $v);
				}
			}
